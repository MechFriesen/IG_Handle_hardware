...
void setup() {  
 ...
 /* configure input pins */
  ...
  analogWriteFrequency(CAM2_OUT, 2.0);  // We're using a PWM signal because it's a way of offloading
  analogWriteFrequency(CAM3_OUT, 2.0);  // the task to free up the main loop
  analogWriteFrequency(CAM4_OUT, 2.0);  // CAM4_OUT (Teensy 3.6 pin 3/physical pin 5) is on FTM1 and
                                         // can be set at a different frequency without affecting the
                                         // other camera trigger pins
